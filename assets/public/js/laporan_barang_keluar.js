$(function () {  
    var show = document.getElementById("barang_keluar")
    ...
    $('#search').click(function () { 
        ...
        show.style.display = "none";
        $("#table_barang_keluar").empty().append()
        $("#print_table_barang_keluar").empty().append()
        
        if(start_date){
            if(end_date){
                $.ajax({
                    url: '../Backend/search_laporan_barang_keluar',
                    type: 'GET',
                    dataType: 'JSON',
                    data: {
                    ...
                    success: function (x) {  
                        console.log(x)
                        if(x.barang_keluar.length > 0 ) {
                            var no = 1
                            $.each(x.barang_keluar, function (k, v) {  
                                var barang_keluar = `
                                    <tr>
                                        <td style="text-align:center">${no}</td>
                                        <td style="text-align:center">${getFormattedDate(v.tgl_keluar)}</td>
                                        <td style="text-align:center">${v.batch_number}</td>
                                        <td style="text-align:center">${v.nama_barang}</td>
                                        <td style="text-align:center">${v.qty_diserahkan}</td>
                                    </tr>
                                `
                                $('#table_barang_keluar').append(barang_keluar).html()
                                $('#print_table_barang_keluar').append(barang_keluar).html()
                                no++
                            })
                            ...
                            `
                            $('#table_barang_keluar').html(barang_keluar)
                            $('#print_table_barang_keluar').html(barang_keluar)
                            show.style.display = "block";
                        }
                    }
                })
